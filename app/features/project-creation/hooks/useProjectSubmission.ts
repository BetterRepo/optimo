...
export const useProjectSubmission = () => {
  ...
  const handleSubmit = async (formData: ProjectCreationFormData) => {
    ...
    try {
      ...
      const ubillFiles =
        Array.isArray(formData.ubillFiles) && formData.ubillFiles.length > 0
      ...
      const hasSecondaryContact =
        !!(formData as any).secondaryFirstName ||
        !!(formData as any).secondaryLastName;
      ...
      try {
        ...
        if (storedSalesCompany) {
          ...
          console.log(
            "Retrieved salesCompany from localStorage:",
            salesCompany
          );
        }
      } catch (error) {
        console.error(
          "Error retrieving salesCompany from localStorage:",
          error
        );
      }
      ...
      const payload = {
        ...
        projectDetails: {
          ...
          adders: Array.isArray(formData.adders)
            ? formData.adders.join(";")
            : formData.adders,
      ...
      console.log(
        "Payload being sent to webhook-proxy:",
