...
// Add new AdditionalNotesSection component just after other similar component definitions
const AdditionalNotesSection: React.FC<{
  value: string;
  onChange: (e: React.ChangeEvent<HTMLTextAreaElement>) => void;
}> = ({ value, onChange }) => {
  return (
    <div className="py-6 mb-4 border-t border-gray-200 dark:border-gray-700">
      <div>
        ...
        <textarea
          ...
          className="w-full px-4 py-3 rounded-lg 
                   ...
                   focus:outline-none focus:border-gray-400 dark:focus:border-gray-500 focus:ring-0"
...
function ProjectCreationFormContent() {
  ...
  >({
    ...
    postalCode: "",
    warehouse: "",
    systemSize: "", // Initialize system size field
    ubillFile: undefined,
    ubillFiles: [],
    ...
    customerPhone: "",
    internalToken: "",
    // Secondary contact fields - initialized as undefined
    secondaryFirstName: "",
    secondaryLastName: "",
    secondaryEmail: "",
    secondaryPhone: "",
    secondaryRelationship: "",
    // Tenant information - initialized as undefined
    hasTenants: undefined,
    tenantFirstName: "",
    tenantLastName: "",
    tenantEmail: "",
    tenantPhone: "",
    additionalNotes: "", // Initialize the additional notes field
  });
  ...
  useEffect(() => {
    ...
    if (salesCompany) {
      try {
        localStorage.setItem("salesCompany", salesCompany);
        console.log("Stored salesCompany parameter:", salesCompany);
      } catch (err) {
  ...
  // Pass this function to the FinanceCompanySelector
  ...
  // Expose the direct toggle function to the global window object (temporary debugging solution)
  useEffect(() => {
    // @ts-expect-error - Adding function to window object for emergency debugging
    window.forceToggleWelcomeCallStatus = forceToggleWelcomeCallStatus;
  }, []);
  ...
  const handleFileUpload = async (files: FileList): Promise<boolean> => {
    try {
      if (files.length === 0) {
    ...
    <form onSubmit={onSubmit} className="space-y-6 pb-10">
      ...
      <div className="pt-8 pb-6">
        ...
        <FinanceCompanySelector
          ...
          selectedSystemSize={formData.systemSize}
          formData={formData}
