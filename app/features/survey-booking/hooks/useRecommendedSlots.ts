...
export const useRecommendedSlots = (formData: any, orderNo: string | null) => {
	...
	const fetchRecommendedSlots = async () => {
		try {
			...
			console.log(
				`RECOMMENDED SLOTS - PST Tomorrow: ${format(
					tomorrow,
					"yyyy-MM-dd"
				)}`
			);
			...
			while (
				!isValidBookingDate(secondDay) ||
				isSameDay(secondDay, firstDay)
			) {
				console.log(
      ...
      `);
			...
			formattedDates = formattedDates.filter((dateString) => {
				if (isFloridaBlackoutDate(dateString)) {
					console.log(
						`Filtering out Florida blackout date: ${dateString}`
					);
					return false;
				}
				return true;
			...
			while (recommendedDates.length < 5) {
				...
				recommendedDates.push(
					additionalWeekday.toISOString().split("T")[0]
				);
				addDay = addDays(addDay, 1);
			}
			...
			const payload = {
				...
				planning: {
					...
					...(driverMapping[formData.warehouse as Warehouse]?.length >
						0 &&
						formData.warehouse !== "Out of Region" && {
							useDrivers: driverMapping[
								formData.warehouse as Warehouse
							].map((driver) => ({
			...
			if (data.orderNo) {
				console.log(
					"Using new orderNo from API response:",
					data.orderNo
				);
			}
