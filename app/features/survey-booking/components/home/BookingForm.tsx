  ...
  useEffect(() => {
    if (formData) {
      handleChange({
        target: {
          id: 'type',
          value: formData.isAdditionalSurvey ? 'Additional Survey Booking' : 'Survey Booking'
        }
      } as React.ChangeEvent<HTMLInputElement>);
    }
  }, [formData.isAdditionalSurvey]);

  const handleRecommendedSlotSelect = (value: string, reservationId: string, orderNo: string) => {
    setSelectedSlot({ 
      value, 
      reservationId,
      orderNo
      ...
      {formData.warehouse === 'Dallas, TX' && (
        <div className="mb-8 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl p-6 shadow-sm border border-blue-100 dark:border-blue-800/40">
          ...
          <div className="grid grid-cols-2 gap-4 mt-4">
            <div 
              className={`flex items-center p-4 ${formData.texasProximity === 'Dallas' 
            ...
            >
              <input
                ...
                onChange={() => {}}
                className={`h-5 w-5 ${formData.texasProximity === 'Dallas' ? 'text-white border-white' : 'text-blue-600 border-gray-300'}`}
              />
              <label htmlFor="texasProximityDallas" className="ml-3 block font-medium cursor-pointer flex-1">
                Dallas
              </label>
              {formData.texasProximity === 'Dallas' && (
            ...
            <div 
              className={`flex items-center p-4 ${formData.texasProximity === 'Houston' 
                ? 'bg-blue-500 text-white' 
                : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200'} 
                rounded-lg shadow-sm cursor-pointer hover:shadow-md transition-all duration-200 border ${
            ...
            >
              <input
                ...
                onChange={() => {}}
                className={`h-5 w-5 ${formData.texasProximity === 'Houston' ? 'text-white border-white' : 'text-blue-600 border-gray-300'}`}
              />
              <label htmlFor="texasProximityHouston" className="ml-3 block font-medium cursor-pointer flex-1">
                Houston
              </label>
              {formData.texasProximity === 'Houston' && (
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-white" viewBox="0 0 20 20" fill="currentColor">
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                <p className="text-amber-800 dark:text-amber-200">
                  <span className="font-medium">Important:</span> Your selection affects available survey days.
                </p>
                <ul className="mt-1 text-amber-700 dark:text-amber-300 list-disc pl-5 space-y-1">
                  <li><span className="font-medium">Dallas area:</span> Surveys on Wednesday and Thursday</li>
                  <li><span className="font-medium">Houston area:</span> Surveys on Monday, Tuesday, and Friday</li>
                  <li><span className="font-medium">Note:</span> Weekends are not available for surveys</li>
                </ul>
              </div>
            </div>
          </div>
      ...
      {formData.warehouse && (
        <div className="">
          {isOutOfRegion ? (
            ...
            <>
              <div className="text-gray-700 dark:text-gray-300 text-sm mb-6 p-3 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800/70 dark:to-gray-800/50 border-l-3 border-red-400 dark:border-red-500 rounded shadow-sm flex items-center">
                <span className="mr-2 text-red-500">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                  </svg>
                </span>
                Homeowner must be present during survey with access to electrical panels, roof, and attic
              </div>
              ...
              {!formData.internal && (
                <>
                  ...
                  <SurveyQuestions formData={formData} handleChange={handleChange} hasSubmitAttempt={hasSubmitAttempt} />
                </>
              )}
              ...
              <div className="mt-8 mb-4 overflow-hidden rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 relative">
                ...
                <div className="relative p-3 z-10">
                  ...
                  <FeedbackSelector 
                    value={formData.feedbackRating as 'happy' | 'neutral' | 'unhappy'} 
                    onChange={(value) => {
                      handleChange({
                        target: {
                          id: 'feedbackRating',
