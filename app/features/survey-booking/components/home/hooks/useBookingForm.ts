...
export const useBookingForm = () => {
  ...
  const [formData, setFormData] = useState<SurveyFormData>({
    ...
    language: searchParams.get("language") === "Spanish" ? "Spanish" : "English",
  ...
  const testWebhook = async () => {
    try {
      ...
      console.log("🧪 DIRECT TEST RESPONSE:", directResponse.status, await directResponse.text());
      ...
      console.log("🧪 PROXY TEST RESPONSE:", proxyResponse.status, await proxyResponse.text());
      
      return "Test webhook sent. Check console for results.";
    } catch (error) {
      console.error("🧪 TEST WEBHOOK ERROR:", error);
      return `Test webhook failed: ${error instanceof Error ? error.message : "Unknown error"}`;
    }
  };
    ...
    try {
      ...
      if (formData.isSurveyBooking) {
        ...
        if (missingFields.length > 0) {
          ...
          setOrderErrorMessage(`Please fill in all required fields: ${missingFieldNames}`);
          setIsSubmitting(false);
          return;
        }
      }
      ...
      if (!formData.isSurveyBooking) {
        ...
        if (formData.hasBatteries === undefined) {
          setOrderErrorMessage("Please answer if batteries are being installed as part of this project.");
          setIsSubmitting(false);
          return;
        }
        // Check if design preference has been selected
        if (!formData.designPreference) {
          setOrderErrorMessage("Please select your preferred design option for panel placement.");
          setIsSubmitting(false);
          return;
        }
        // Check if shared roof question has been answered
        if (formData.isSharedRoof === undefined) {
          setOrderErrorMessage("Please indicate if this property has a shared roof with other units.");
          setIsSubmitting(false);
          return;
        }
        // Check if recent construction question has been answered
        if (formData.hasRecentConstruction === undefined) {
          setOrderErrorMessage("Please indicate if there has been recent construction affecting the roof.");
          setIsSubmitting(false);
          return;
        }
        // Check if vaulted ceilings question has been answered
        if (formData.hasVaultedCeilings === undefined) {
          setOrderErrorMessage("Please indicate if there are vaulted ceilings in the home.");
          setIsSubmitting(false);
          return;
        }
        // Check if stories has been selected
        if (!formData.stories) {
          setOrderErrorMessage("Please select the number of stories in the home.");
          setIsSubmitting(false);
          return;
        }
        // Check if ongoing construction question has been answered
        if (formData.hasOngoingConstruction === undefined) {
          setOrderErrorMessage("Please indicate if there is any ongoing construction, open permits or un-permitted structures on the property.");
          setIsSubmitting(false);
          return;
        }
        // Check if existing solar question has been answered
        if (formData.hasExistingSolar === undefined) {
          setOrderErrorMessage("Please indicate if there is an existing solar system on the property.");
          setIsSubmitting(false);
          return;
        }
        // Check if HOA question has been answered
        if (formData.hasHOA === undefined) {
          setOrderErrorMessage("Please indicate if there is an HOA associated with this property.");
          setIsSubmitting(false);
          return;
        }
        // Check if main panel location has been selected
        if (!formData.mainPanelLocation) {
          setOrderErrorMessage("Please indicate where your main electrical panel is located.");
          setIsSubmitting(false);
          return;
        }
        // If "other" is selected for mainPanelLocation, check if details are provided
        if (formData.mainPanelLocation === "other" && !formData.mainPanelOtherLocation) {
          setOrderErrorMessage("Please specify the location of your main electrical panel.");
          setIsSubmitting(false);
          return;
        }
        ...
        if (formData.hasBlockedAccess === undefined) {
          setOrderErrorMessage("Please indicate if access to the home or electrical panel is blocked.");
          setIsSubmitting(false);
          return;
        }
        // Check if gate access type has been selected
        if (!formData.gateAccessType) {
          setOrderErrorMessage("Please indicate if you have an access code for the neighborhood/property.");
          setIsSubmitting(false);
          return;
        }
        // If "code" is selected for gateAccessType, check if code is provided
        if (formData.gateAccessType === "code" && !formData.gateCode) {
          setOrderErrorMessage("Please enter the access code for the neighborhood/property.");
          setIsSubmitting(false);
          return;
        }
      }
      ...
      const updatedFormData = {
        ...
        firstName: formData.firstName || (formData.isSurveyBooking ? "Survey" : ""),
        lastName: formData.lastName || (formData.isSurveyBooking ? "Customer" : ""),
        // Set default address if missing for survey booking
        streetAddress: formData.streetAddress || (formData.isSurveyBooking ? "123 Main St" : ""),
        city: formData.city || (formData.isSurveyBooking ? "Los Angeles" : ""),
        state: formData.state || (formData.isSurveyBooking ? "CA" : ""),
        postalCode: formData.postalCode || (formData.isSurveyBooking ? "90001" : ""),
        warehouse: formData.warehouse || (formData.isSurveyBooking ? "Cerritos, CA" : ""),
        ...
        hasBatteriesText: formData.hasBatteriesText || (formData.hasBatteries === true ? "Yes" : formData.hasBatteries === false ? "No" : ""),
        includeStorageText: formData.includeStorageText || (formData.includeStorage === true ? "Yes" : formData.includeStorage === false ? "No" : ""),
        designPreferenceText: formData.designPreferenceText || 
          (formData.designPreference === "bestSunlight" ? "Design the system for maximum efficiency" : 
            formData.designPreference === "matchProposed" ? "Design the system to match the proposal design as closely as possible" : ""),
        isSharedRoofText: formData.isSharedRoofText || (formData.isSharedRoof === true ? "Yes, please place the design system exactly as specified" : formData.isSharedRoof === false ? "No" : ""),
        hasRecentConstructionText: formData.hasRecentConstructionText || (formData.hasRecentConstruction === true ? "Yes" : formData.hasRecentConstruction === false ? "No" : ""),
        hasHOAText: formData.hasHOAText || (formData.hasHOA === true ? "Yes" : formData.hasHOA === false ? "No" : ""),
        hasHOAContactText: formData.hasHOAContactText || (formData.hasHOAContact === true ? "Yes" : formData.hasHOAContact === false ? "No" : ""),
        hasVaultedCeilingsText: formData.hasVaultedCeilingsText || (formData.hasVaultedCeilings === true ? "Yes" : formData.hasVaultedCeilings === false ? "No" : ""),
        hasOngoingConstructionText: formData.hasOngoingConstructionText || (formData.hasOngoingConstruction === true ? "Yes" : formData.hasOngoingConstruction === false ? "No" : ""),
        hasExistingSolarText: formData.hasExistingSolarText || (formData.hasExistingSolar === true ? "Yes" : formData.hasExistingSolar === false ? "No" : ""),
        hasSkylightsText: formData.hasSkylightsText || (formData.hasSkylights === true ? "Yes" : formData.hasSkylights === false ? "No" : ""),
        hasOutsidePanelText: formData.hasOutsidePanelText || (formData.hasOutsidePanel === true ? "Yes" : formData.hasOutsidePanel === false ? "No" : ""),
        hasSubPanelsText: formData.hasSubPanelsText || (formData.hasSubPanels === true ? "Yes" : formData.hasSubPanels === false ? "No" : ""),
        hasBlockedAccessText: formData.hasBlockedAccessText || (formData.hasBlockedAccess === true ? "Yes" : formData.hasBlockedAccess === false ? "No" : ""),
        gateAccessTypeText: formData.gateAccessTypeText || 
          (formData.gateAccessType === "yes" ? "Yes, I'll be there to provide access" : 
           formData.gateAccessType === "code" ? "Yes, I have a code" : 
           formData.gateAccessType === "no" ? "No" : ""),
        mainPanelLocationText: formData.mainPanelLocationText || 
          (formData.mainPanelLocation === "inside" ? "Inside the home" : 
           formData.mainPanelLocation === "outside" ? "Outside the home" :
           formData.mainPanelLocation === "garage" ? "In the garage" :
           formData.mainPanelLocation === "other" ? "Other location" : ""),
        petsText: formData.petsText || (formData.pets === true ? "Yes" : formData.pets === false ? "No" : ""),
      };
      ...
      console.log('   selectedDate:', selectedDate ? selectedDate.toISOString() : null);
      console.log('   Actual survey date (no timezone conversion):', actualSurveyDate ? actualSurveyDate.toISOString() : null);
      
      // Format as YYYY-MM-DD using local date components to avoid timezone issues
      const actualSurveyDateStr = actualSurveyDate 
        ? `${actualSurveyDate.getFullYear()}-${String(actualSurveyDate.getMonth() + 1).padStart(2, '0')}-${String(actualSurveyDate.getDate()).padStart(2, '0')}`
        : null;
      
      console.log('   Formatted survey date string (local time):', actualSurveyDateStr);
      ...
      try {
        ...
        const surveyBookingPayload = {
          ...
          streetAddress: formData.streetAddress ? 
                        `${formData.streetAddress ? formData.streetAddress.replace(/Street/i, "St") : ""}, ${formData.city}, ${formData.state} ${formData.postalCode}, USA` : 
          ...
          hasOngoingConstruction: formData.hasOngoingConstruction ? "Yes" : "No",
        ...
        console.log("📩 SENDING SURVEY BOOKING DATA TO ZAPIER:", JSON.stringify(surveyBookingPayload, null, 2));
      ...
      // Get a proper customer name from multiple sources
      const fullName = `${updatedFormData.firstName} ${updatedFormData.lastName}`.trim();
      const customerName = fullName || updatedFormData.projectName || updatedFormData.customerEmail || updatedFormData.customerPhone || updatedFormData.quickbaseRecordId || "Customer";
      const warehouse = updatedFormData.warehouse;
      
      // Format the address string
      const formattedAddress = `${updatedFormData.streetAddress ? updatedFormData.streetAddress.replace(/Street/i, "St") : ""}, ${updatedFormData.city}, ${updatedFormData.state} ${updatedFormData.postalCode}, USA`;
      
      // First create or get location - this is the new step to fix pin placement issues
      const geocodedCoordinates = (formData.latitude && formData.longitude) ? {
      ...
      console.log(`📅 Using ${timeWindows.length} time windows for warehouse: ${warehouse}`, timeWindows);

      // Get drivers for the warehouse
      const drivers = warehouse !== "Out of Region" ? driverMapping[warehouse as Warehouse] || [] : [];
      console.log(`👥 Using ${drivers.length} drivers for warehouse: ${warehouse}`, drivers);
      ...
      const bookingData = {
        order: {
          ...
          type: "D",
          email: updatedFormData.email || updatedFormData.customerEmail || "",
          phone: updatedFormData.phone || updatedFormData.customerPhone || "",
          customField1: customerName || fullName, // Use customer name as customField1
          // Add the selected time window directly to the order
          timeWindows: [{
            twFrom: fromTime,
            twTo: toTime
          }],
          // Add driver assignment using correct OptimoRoute format
          ...(drivers.length > 0 && warehouse !== "Out of Region" && {
            assignedTo: {
              externalId: drivers[0].driverExternalId // Assign to first available driver
            }
          }),
          ...
          date: actualSurveyDateStr,
          duration: 60,
          // Add tags for proper categorization
          tags: ["survey", warehouse?.replace(', ', '_') || "out_of_region"],
          // Add status to ensure it's scheduled
          status: "NEW"
        },
        slots: {
          dates: [actualSurveyDateStr],
          // Use only the selected time window for slot booking
          timeWindows: [{
            twFrom: fromTime,
            twTo: toTime
          }]
        ...
        texasProximity: formData.warehouse === 'Dallas, TX' ? formData.texasProximity : undefined,
        warehouse: warehouse,
      ...
      const reservationResponse = await fetch("/api/MakeReservation", {
        ...
        body: JSON.stringify({
          reservationId: selectedSlot.reservationId,
          createOrderIfBookingFails: formData.isSurveyBooking, // Only true for CRM bookings
          // Include order data for direct creation
      ...
      if (!reservationResponse.ok || (reservationData && !reservationData.success)) {
        console.error('Reservation failed:', reservationData);
        throw new Error(`Reservation failed: ${reservationData?.message || 'Unknown error'}`);
      }

      // If order was created directly (for CRM bookings), skip the update step
      if (reservationData.orderCreated) {
        console.log('✅ Order created directly, skipping redundant update step');
        console.log('   Order details:', reservationData.orderDetails);
      } else {
        // Wait for order creation
        await new Promise(resolve => setTimeout(resolve, 1000));

        // Step 3: Update time windows (only if order wasn't created directly)
        console.log('Updating order with:', {
          email: updatedFormData.email || updatedFormData.customerEmail || "",
          phone: updatedFormData.phone || updatedFormData.customerPhone || "",
        });

        // Extract the selected time window from selectedSlot.value
        const [finalFromDateTime, finalToDateTime] = selectedSlot.value.split(' - ');
        const selectedFromTime = finalFromDateTime.split('T')[1].substring(0, 5);
        const selectedToTime = finalToDateTime.split('T')[1].substring(0, 5);
        
        console.log(`📅 Using ONLY selected time window: ${selectedFromTime} - ${selectedToTime}`);

        // When updating the order, use the same location approach
        const updatePayload = {
            orders: [{
              operation: "UPDATE",
              orderNo: selectedSlot.orderNo,
              timeWindows: [{
                twFrom: selectedFromTime,
                twTo: selectedToTime
              }],
              // Add driver assignment using correct OptimoRoute format
              ...(drivers.length > 0 && warehouse !== "Out of Region" && {
              ...
              email: updatedFormData.email || updatedFormData.customerEmail || "",
              phone: updatedFormData.phone || updatedFormData.customerPhone || "",
              customField1: `Survey ${updatedFormData.lastName}`,
              duration: 60,
              location: {
                locationNo: locationData.locationNo,
                locationName: customerName,
                address: formattedAddress,
                // CRITICAL FIX: Use coordinates from locationData instead of formData
                // This ensures we use the actual geocoded coordinates for the specific address
      ...
      // Store booking recap and redirect
      // Use the actualSurveyDate from earlier (now same as selectedDate)
      console.log('🔄 CREATING BOOKING RECAP:');
      console.log('   Order number:', selectedSlot.orderNo);
      console.log('   Selected date:', selectedDate ? selectedDate.toISOString() : null);
      console.log('   Actual survey date (same as selected):', actualSurveyDate ? actualSurveyDate.toISOString() : null);
      console.log('   Actual survey date string:', actualSurveyDateStr);
      ...
      console.log('📝 Final booking recap object (before stringify):', JSON.stringify({
      ...
      const dateParam = bookingRecap.slotDate ? `?date=${encodeURIComponent(bookingRecap.slotDate)}` : '';
      ...
      const webhookPayload = {
        ...
        projectDetails: {
          ...
          address: `${formData.streetAddress ? formData.streetAddress.replace(/Street/i, "St") : ""}, ${formData.city}, ${formData.state} ${formData.postalCode}`,
          warehouse: formData.warehouse,
      ...
      try {
        ...
        const zapierErrorPayload = {
          ...
          token: `TPO-ERR-${Date.now()}-${Math.random().toString(36).substring(2, 10)}`,
          firstName: formData.firstName,
          lastName: formData.lastName,
          email: formData.email || formData.customerEmail || "",
          phone: formData.phone || formData.customerPhone || "",
          ...
          designPreference: formData.designPreferenceText || 
            (formData.designPreference === "bestSunlight" ? "Design the system for maximum efficiency" : 
              formData.designPreference === "matchProposed" ? "Design the system to match the proposal design as closely as possible" : 
              formData.designPreference),
          hasBatteries: formData.hasBatteriesText || (formData.hasBatteries === true ? "Yes" : formData.hasBatteries === false ? "No" : ""),
          includeStorage: formData.includeStorageText || (formData.includeStorage === true ? "Yes" : formData.includeStorage === false ? "No" : ""),
          // Add other important fields in the same format
          formDataJson: JSON.stringify(formData)
        };
        
        console.log("Zapier error payload:", JSON.stringify(zapierErrorPayload, null, 2));
        ...
        console.log('Zapier error webhook response status:', zapierErrorResponse.status);
        console.log('Zapier error webhook response text:', errorResponseText);
        
        console.log('Zapier error webhook response:', zapierErrorResponse.ok ? 'Success' : 'Failed');
      } catch (webhookError) {
        console.error('Failed to log to webhook:', webhookError);
      }

      setOrderErrorMessage(error instanceof Error ? error.message : "Failed to submit booking");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const onDateSelect = async (date: Date | undefined) => {
    ...
    const convertToUSDate = (date: Date): Date => {
      ...
      const easternDate = new Date(`${yearPart}-${monthPart}-${dayPart}T12:00:00`);
      
      console.log('Date conversion:', {
        originalDate: date.toISOString(),
        easternDateComponents: { year: yearPart, month: monthPart, day: dayPart },
    ...
    console.log('Formatted date for API (original calendar date):', formattedDate);

    // Generate a more random order number to avoid conflicts
    const timestamp = Date.now();
    const randomString = Math.random().toString(36).substring(2, 8).toUpperCase();
    const generatedOrderNo = `BE${timestamp}${randomString}`;
    setSelectedSlot({ value: "", reservationId: "", orderNo: generatedOrderNo });
    ...
    const streetAddress = formData.streetAddress || (formData.isSurveyBooking ? "123 Main St" : "");
    const city = formData.city || (formData.isSurveyBooking ? "Los Angeles" : "");
    const state = formData.state || (formData.isSurveyBooking ? "CA" : "");
    const postalCode = formData.postalCode || (formData.isSurveyBooking ? "90001" : "");
    const warehouse = formData.warehouse || (formData.isSurveyBooking ? "Cerritos, CA" : "Out of Region");
    
    // Get drivers based on warehouse if possible
    const drivers = (warehouse && warehouse !== "Out of Region") ? 
    ...
    const bookingData = {
      ...
      if (response.ok) {
        ...
        if (responseData.success) {
          ...
          if (responseData.orderNo) {
            console.log('Updating orderNo from API response:', responseData.orderNo);
      ...
      setAvailableSlots([]);
      setSlotErrorMessage(error.message || "An unexpected error occurred while fetching slots.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSlotSelect = async (value: string, reservationId: string) => {
    try {
      ...
      console.log('✅ Slot selection completed - order will be created during submission');
