...
export function DatePicker({ onSelect, warehouse }: DatePickerProps) {
  ...
  function isPastCutoffTime() {
    ...
    console.log(`DatePicker - Current PST time: ${pstDate.toLocaleTimeString()}, Hour: ${pstDate.getHours()}, Past 5 PM: ${isPast5PM}`);
  ...
  console.log(`DatePicker - Minimum advance days: ${minAdvanceDays} (cutoff passed: ${isCutoff})`);
  ...
  function isDateDisabled(date: Date) {
    ...
    if (isFloridaBlackoutDate) {
      console.log(`DatePicker - Date ${format(date, 'yyyy-MM-dd')} disabled because: Florida blackout date`);
    }
    
    const isDisabled = isWeekend || isTooEarly || isFloridaBlackoutDate;
    
    if (isDisabled) {
      console.log(`DatePicker - Date ${format(date, 'yyyy-MM-dd')} disabled because: ${
        isWeekend ? 'weekend' : (isTooEarly ? 'too early (cutoff rule)' : (isFloridaBlackoutDate ? 'Florida blackout date' : 'unknown'))
      }`);
    }
  ...
  while (isDateDisabled(defaultDate)) {
    console.log(`DatePicker - Default date ${format(defaultDate, 'yyyy-MM-dd')} is invalid, adding one day`);
    defaultDate = addDays(defaultDate, 1);
  }
  
  console.log(`DatePicker - Final default date: ${format(defaultDate, 'yyyy-MM-dd')}`);
  ...
  function handleSelect(selectedDate: Date | undefined) {
    if (selectedDate && isDateDisabled(selectedDate)) {
      console.error(`DatePicker - Prevented selection of disabled date: ${format(selectedDate, 'yyyy-MM-dd')}`);
      ...
      <Popover>
        ...
        <PopoverContent className="w-auto p-0" align="start">
          <Calendar
            ...
            disabled={isDateDisabled} // Use our comprehensive date disabling function
            initialFocus
          />
        </PopoverContent>
      </Popover>
      
      {/* Cutoff time indicator - only shown when cutoff is active */}
