...
const expandAddressAbbreviations = (address: string): string => {
  ...
  const expandedWords = words.map(word => {
    ...
    if (ADDRESS_ABBREVIATIONS[cleanWord]) {
      ...
      return word.replace(new RegExp(cleanWord, 'i'), ADDRESS_ABBREVIATIONS[cleanWord]);
    }
...
export const AddressForm: React.FC<AddressFormProps> = ({
  ...
  useEffect(() => {
    const isAddressValid = formData.streetAddress && 
                          formData.city && 
                          formData.state && 
                          formData.postalCode;
  ...
  const handleGooglePlaceSelect = async (place: google.maps.places.PlaceResult) => {
    ...
    const { streetAddress, city, state, postalCode } = parseGoogleAddress(place);
    ...
    try {
      ...
      console.log('🔄 Calling geocoding API for selected address:', fullAddress);
  ...
  return (
    <div className="space-y-8">
      <div className="mt-6 relative">
        <label htmlFor="streetAddress" className="block text-[#1E293B] dark:text-white text-base font-semibold mb-3">
        ...
        <div>
          ...
          <div className="relative">
            ...
            >
              ...
              <option value="TX" className="text-gray-900 dark:text-white">TX</option>
              <option value="CA" className="text-gray-900 dark:text-white">CA</option>
              <option value="FL" className="text-gray-900 dark:text-white">FL</option>
              <option value="AZ" className="text-gray-900 dark:text-white">AZ</option>
              <option value="Out of Region" className="text-gray-900 dark:text-white">Out of Region</option>
            </select>
        ...
        <div>
          ...
          <div className="relative">
            ...
            >
              ...
              {WAREHOUSES.filter(w => w !== '').map((warehouse) => (
                <option key={warehouse} value={warehouse} className="text-gray-900 dark:text-white">
                  {warehouse}
                </option>
              ))}
            </select>
          ...
          {formData.warehouse && (
            <p className="mt-3 text-sm text-gray-500 dark:text-gray-400 flex items-center">
              ...
              Warehouse is automatically determined by your zip code and cannot be changed.
            </p>
          )}
        </div>
      </div>
