...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('📝 Project submit API called with data:', JSON.stringify(formData, null, 2));
    ...
    if (formData.hasSecondaryContact || formData.secondaryFirstName || formData.secondaryLastName) {
    ...
    if (formData.hasTenants || formData.tenantFirstName || formData.tenantLastName) {
    ...
    if (!formData.firstName || !formData.lastName || !formData.customerEmail) {
      return NextResponse.json({
        success: false,
        message: "Missing required fields: firstName, lastName, or customerEmail"
      }, { status: 400 });
    }
    ...
    if (formData.ubillFiles) {
      console.log('📝 Received ubillFiles:', JSON.stringify(formData.ubillFiles, null, 2));
    ...
    // Forward to Zapier
    try {
      const zapierUrl = 'https://hooks.zapier.com/hooks/catch/16426358/2wogpdo/';
      ...
      if (!payloadToSend.leadType && formData.projectDetails?.leadType) {
        ...
        console.log('📝 Extracted leadType from projectDetails:', payloadToSend.leadType);
      }
      
      // In case financeType isn't included, add it to the payload if available in formData
      if (!payloadToSend.financeType) {
        console.log('📝 Warning: financeType not found at top level of payload');
      }
      ...
      if (!payloadToSend.projectDetails) {
        ...
        payloadToSend = {
          ...
          hasSecondaryContact: !!payloadToSend.secondaryFirstName || !!payloadToSend.secondaryLastName,
          secondaryContact: (payloadToSend.secondaryFirstName || payloadToSend.secondaryLastName) ? {
          ...
          hasTenants: !!payloadToSend.tenantFirstName || !!payloadToSend.tenantLastName || !!payloadToSend.hasTenants,
          tenant: (payloadToSend.tenantFirstName || payloadToSend.tenantLastName || payloadToSend.hasTenants) ? {
          ...
          ubillFiles: payloadToSend.ubillFiles,
          ubillFilesUrls: Array.isArray(payloadToSend.ubillFiles) && payloadToSend.ubillFiles.length > 0 
            ? payloadToSend.ubillFiles.map((file: any) => file.url).join(';') 
            : "",
          ubillFilesNames: Array.isArray(payloadToSend.ubillFiles) && payloadToSend.ubillFiles.length > 0 
            ? payloadToSend.ubillFiles.map((file: any) => file.name).join(';') 
            : "",
          // Include Google Drive viewer URLs (not direct download URLs)
          ubillFilesDirectUrls: Array.isArray(payloadToSend.ubillFiles) && payloadToSend.ubillFiles.length > 0 
            ? payloadToSend.ubillFiles.map((file: any) => {
        ...
        const hasSecondaryContact = !!payloadToSend.secondaryFirstName || 
        ...
        const hasTenants = !!payloadToSend.tenantFirstName || 
        ...
        payloadToSend = {
          ...
          hasSecondaryContact,
          secondaryFirstName: payloadToSend.secondaryFirstName || payloadToSend.secondaryContact?.firstName || '',
          secondaryLastName: payloadToSend.secondaryLastName || payloadToSend.secondaryContact?.lastName || '',
          secondaryEmail: payloadToSend.secondaryEmail || payloadToSend.secondaryContact?.email || '',
          secondaryPhone: payloadToSend.secondaryPhone || payloadToSend.secondaryContact?.phone || '',
          secondaryRelationship: payloadToSend.secondaryRelationship || payloadToSend.secondaryContact?.relationship || '',
          // Ensure tenant info is present at top level
          hasTenants,
          tenantFirstName: payloadToSend.tenantFirstName || payloadToSend.tenant?.firstName || '',
          tenantLastName: payloadToSend.tenantLastName || payloadToSend.tenant?.lastName || '',
          tenantEmail: payloadToSend.tenantEmail || payloadToSend.tenant?.email || '',
          tenantPhone: payloadToSend.tenantPhone || payloadToSend.tenant?.phone || '',
          // Add ubillFiles at the top level for Zapier to easily access
          ubillFiles: ubillFiles,
          ubillFilesUrls: Array.isArray(ubillFiles) && ubillFiles.length > 0 
            ? ubillFiles.map((file: any) => file.url).join(';') 
            : "",
          ubillFilesNames: Array.isArray(ubillFiles) && ubillFiles.length > 0 
            ? ubillFiles.map((file: any) => file.name).join(';') 
            : "",
          // Include Google Drive viewer URLs (not direct download URLs)
          ubillFilesDirectUrls: Array.isArray(ubillFiles) && ubillFiles.length > 0 
            ? ubillFiles.map((file: any) => {
                // Convert any direct download URLs to view URLs
                const url = file.url;
                if (url && url.includes('drive.google.com')) {
                  // Check if it's a direct download URL
      ...
      console.log('📝 Final Zapier payload:', JSON.stringify(payloadToSend, null, 2));
